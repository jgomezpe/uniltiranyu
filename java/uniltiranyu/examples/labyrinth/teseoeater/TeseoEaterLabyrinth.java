...
public class TeseoEaterLabyrinth extends Labyrinth {
  ...
  protected LabyrinthPercept getPercept( int x, int y ){
    ...
    perc.set(LabyrinthUtil.ENERGY, energy_level);
    return perc;
  }
  ...
  public TeseoEaterLabyrinth( Array<Agent> _agents, int[][] _structure ) {
  ...
  public boolean act( Agent agent, Action action ){
    ...
    if( flag ){
      ...
      switch (actionID) {
        case 4: // eat
          if ( ( (Boolean) p.get(LabyrinthUtil.RESOURCE[0])).booleanValue()) {
